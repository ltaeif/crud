...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
{
	...
	if($file_res){
		...
		if (file_exists("sons2/" . $_FILES["file"]["name"]))
		  {
		...
		else
		  {
		   ...
		   move_uploaded_file($_FILES["file"]["tmp_name"],"sons2/" .$new_file_name );
		   echo "Stored in: " . "sons2/" . $new_file_name ;
...
if ( null==$id ) {
			...
			$sql = "INSERT INTO "._DB_PREFIX_."projet (mp3,ajoutdate) values( ?, ?)";
			$q = $pdo->prepare($sql);
			$date = date('Y-m-d');
			...
			try { 
				...
				print "Error!: " . $e->getMessage() . "</br>"; 
					...
					$pdo = Database::connect();				
					$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					$sql = "UPDATE "._DB_PREFIX_."projet  set mp3 = ? , ajoutdate = ?  WHERE id = ?";
					$q = $pdo->prepare($sql);
					$date = date('Y-m-d');
