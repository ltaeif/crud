...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null, 
		$newdirectories= array('upload_dir'=>'/../albums/','urlapp'=>'/../albums/')) {
        $this->response = array();
		$this->newdirectories=$newdirectories;
        $this->options = array(
            'script_url' => $this->get_full_url().'/'.basename($this->get_server_var('SCRIPT_NAME')),
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).$newdirectories['upload_dir'],
            'upload_url' => $this->get_full_url().$newdirectories['urlapp'],
            'user_dirs' => false,
            'mkdir_mode' => 0777,
            'param_name' => 'files',
        ...
        if ($options) {
			...
			//echo $this->options['demande_codedem'];exit;
    ...
    protected function initialize() {
        switch ($this->get_server_var('REQUEST_METHOD')) {
            ...
            case 'POST':
                ...
                break;
            case 'DELETE':
                $this->deletedb($this->options['print_response']);
                break;
            default:
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->name).'&dir='.$this->newdirectories['upload_dir'];
   ...
   protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
    ...
    public function post($print_response = true) {
        if ($this->get_query_param('_method') === 'DELETE') {
            return $this->deletedb($print_response);
        }
    ...
    public function delete($print_response = true) {
	  ...
	  public function deletedb($print_response = true) {
        $response = $this->delete(false);
        foreach ($response as $name => $deleted) {
                ...
                $query->execute();
				 ...
				 $file = $this->newdirectories['upload_dir'].$name;
