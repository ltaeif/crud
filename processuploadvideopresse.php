	...
	if($file_res){
		...
		if (file_exists("videos/" . $_FILES["file"]["name"]))
		...
		else
		  {
		   ...
		   move_uploaded_file($_FILES["file"]["tmp_name"],"videos/" .$new_file_name );
		   echo "Stored in: " . "videos/" . $new_file_name ;
...
if ( null==$id ) {
			...
			$sql = "INSERT INTO "._DB_PREFIX_."presse (video,ajoutdate) values( ?, ?)";
			$q = $pdo->prepare($sql);
			$date = date('Y-m-d');
			...
			try { 
				...
				try { 
					...
					//echo _ASS_UPLOAD_SON_PRESSE_DIR_."$video";
					//delete from folder
				  if(file_exists(_ASS_UPLOAD_SON_PRESSE_DIR_."$video")) 
					 {
					$result =  @unlink(_ASS_UPLOAD_SON_PRESSE_DIR_."$video"); 				
				...
				print "Error!: " . $e->getMessage() . "</br>"; 
					...
					$pdo = Database::connect();				
					$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					$sql = "UPDATE "._DB_PREFIX_."presse  set video = ? , ajoutdate = ?  WHERE id = ?";
					$q = $pdo->prepare($sql);
					$date = date('Y-m-d');
